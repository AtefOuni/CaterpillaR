...
Model_and_Plot <- function(S, S_Name, OOS = 0, FOBS = 0, Model, Horizon_year = 1, Xreg = NULL, Xreg_Name = "", Dummy = FALSE, Save_Plot = FALSE) {
...
if (Model == "arima") {
  ...
  p.fit <- p.fit +  scale_x_continuous(breaks = round(Xmin):ceiling(Xmax_f))+
    ...
    Title <- paste(S_Name, Arima_Model_t)
...
if (Model == "arimax") {
  ...
  if(!is.null(ncol(My_ArimaX))){
    ...
    for(i in 2:ncol(Xreg)) {
      ...
      Res <- forecast(auto.arima(My_ArimaX[,i]), h = H)$mean
  ...
  Ymin <- min(S_0.df$y, S.df.fit_fore$f_lo_95, na.rm = TRUE)
  Ymax <- max(S_0.df$y, S.df.fit_fore$f_up_95, na.rm = TRUE)
